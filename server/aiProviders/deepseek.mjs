...
export async function generateVocabularyDraftWithDeepSeek({ profile, settings, fetchImpl = fetch, timeoutMs }) {
  ...
  const safeProfile = {
    ...
    batchIndex: Math.max(1, Math.round(Number(profile?.batchIndex ?? 1) || 1)),
    batchFocus: truncateInline(profile?.batchFocus, 500),
    generationRange: truncateInline(profile?.generationRange, 80),
  }
    ...
    messages: [
      {
        ...
        content: [
          ...
          'Hard requirements: return NEW entries only; never return an entry listed in existing words.',
          'If the obvious phrases are already used, broaden to adjacent practical expressions for the same user goal instead of returning duplicates.',
          'Return as close to the requested count as possible in this JSON response.',
        ].join('\n'),
      },
      {
        role: 'user',
        content: [
          ...
          `Generation range: ${safeProfile.generationRange || `batch ${safeProfile.batchIndex}`}`,
          safeProfile.batchFocus ? `Batch focus: ${safeProfile.batchFocus}` : '',
...
export async function generateWordRepairsWithDeepSeek({ words, language = '目标语言', settings, fetchImpl = fetch }) {
  if (!settings?.apiKey) {
    throw createHttpError(400, 'DeepSeek API Key 未配置')
  }

  const safeWords = normalizeRepairWords(words)
  if (!safeWords.length) return []
    ...
    messages: [
      {
        ...
        content: [
          '你是多语言词库质检和补全助手。',
          '只返回合法 JSON，不要返回 markdown。',
          'JSON 格式必须为 {"repairs":[{"wordId":"...","word":"...","phonetic":"...","partOfSpeech":"...","meaningZh":"...","exampleEn":"...","exampleZh":"...","tags":"...","difficulty":1}]}。',
          '只补全缺失字段，不要改写用户已经提供的字段。',
          'meaningZh 和 exampleZh 使用中文；exampleEn 使用目标语言例句。',
          'phonetic 可填写目标语言常用读音标注，例如 IPA、假名、罗马音、拼音或常见转写。',
          '不能确定的字段返回空字符串；不要编造专有名词含义。',
        ].join('\n'),
      },
      {
        role: 'user',
        content: [
          `目标语言：${language || '目标语言'}`,
          '请检查下面词库词条，只为 missingFields 中列出的字段生成补全值。',
          JSON.stringify({ words: safeWords }),
        ].join('\n'),
      },
    ],
  ...
  return parseDeepSeekWordRepairs(content, safeWords)
}
...
export function parseDeepSeekWordRepairs(content, words = []) {
  const parsed = parseJsonContent(content)
  const list = Array.isArray(parsed) ? parsed : (parsed?.repairs ?? parsed?.words)
  const allowedIds = new Set(words.map((word) => word.wordId))

  return (Array.isArray(list) ? list : [])
    .map((item) => ({
      wordId: stringValue(item.wordId ?? item.id).trim(),
      word: truncateInline(item.word, 80),
      phonetic: item.phonetic ? normalizePhonetic(item.phonetic) : '',
      partOfSpeech: truncateInline(item.partOfSpeech ?? item.pos, 40),
      meaningZh: truncateInline(item.meaningZh ?? item.meaning ?? item.translation, 120),
      exampleEn: truncateInline(item.exampleEn, 180),
      exampleZh: truncateInline(item.exampleZh, 180),
      tags: truncateInline(item.tags, 120),
      difficulty: Math.max(1, Math.min(5, Math.round(Number(item.difficulty ?? 1) || 1))),
    }))
    .filter((item) => item.wordId && allowedIds.has(item.wordId) && (
      item.phonetic
      || item.partOfSpeech
      || item.meaningZh
      || item.exampleEn
...
function normalizeRepairWords(words) {
  return (Array.isArray(words) ? words : []).slice(0, 80).map((word) => {
    const item = {
      wordId: stringValue(word.wordId ?? word.id).trim(),
      word: truncateInline(word.word, 80),
      phonetic: truncateInline(word.phonetic, 80),
      partOfSpeech: truncateInline(word.partOfSpeech, 40),
      meaningZh: truncateInline(word.meaningZh, 120),
      exampleEn: truncateInline(word.exampleEn, 180),
      exampleZh: truncateInline(word.exampleZh, 180),
      tags: truncateInline(word.tags, 120),
      difficulty: Math.max(1, Math.min(5, Math.round(Number(word.difficulty ?? 1) || 1))),
    }
    const missingFields = []
    if (!item.meaningZh) missingFields.push('meaningZh')
    if (!item.phonetic) missingFields.push('phonetic')
    if (!item.partOfSpeech) missingFields.push('partOfSpeech')
    if (!item.exampleEn) missingFields.push('exampleEn')
    if (!item.exampleZh) missingFields.push('exampleZh')
    return { ...item, missingFields }
  }).filter((word) => word.wordId && word.word && word.missingFields.length)
}
